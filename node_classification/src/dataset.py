...
class StockDataset():
    ...
    def read(self, ):
        ...
        if self.config.model_type == 'HATS':
            with open(os.path.join(self.rel_data_dir, 'adj_mat.pkl'), 'rb') as f:
                self.rel_mat = pickle.load(f)[self.use_rel_list]  # (#rel, #node, #node)
                #self.rel_mat = pickle.load(f)[self.use_rel_list][:,:,:self.neighbors_sample]
                self.neighbors = []  # len = #rel, each element is a list of len = #node, each containing the index of neighbors of the node for the relationship
                for rel_idx, rel_mat_i in enumerate(self.rel_mat):  # rel_mat_i: (#node, #node)
                    rel_neighbors = []
        ...
        with open(os.path.join(self.rel_data_dir, 'rel_num.pkl'), 'rb') as f:
            self.rel_num = pickle.load(f)[self.use_rel_list]  # (#rel, #node), the number of neighbors of the node for each relation
            self.num_relations = len(self.rel_num)
        if self.config.model_type == 'HATS':
            k = self.neighbors_sample
            self.rel_num = (self.rel_num>=k)*k + (self.rel_num<k)*self.rel_num  # cropped to max=20

        tot_days = 1652  # TODO: magic value
        tot_ph = int(tot_days/self.config.test_size)
        ...
        for ticker in ordered_ticker:
            ...
            # TODO: problematic starting point
            ...
            self.dev_label.append(np.expand_dims(label_df.iloc[dev_start_idx:dev_end_idx].values, 1))
            self.test_set.append(df.iloc[test_start_idx:test_end_idx].values)
            self.test_label.append(np.expand_dims(label_df.iloc[test_start_idx:test_end_idx].values, 1))
        ...
        #     tmp_th += th
        self.threshold = [-0.004, 0.004, 0.5]
    ...
    def batch_for_aligned_days(self, data, label_data, lookback):
        ...
        for w_start in range(data.shape[1] - lookback):
            mv_percent = label_data[w_start+lookback]
            all_mv.append(mv_percent)
