...
class HATS(BaseModel):
    ...
    def get_state(self, state_module):
        if state_module == 'lstm':
            ...
            outputs, state = tf.nn.dynamic_rnn(lstm_cell, self.x, dtype=tf.float32)  # state: hidden state
    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            ...
            self.accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))  # seems not used
